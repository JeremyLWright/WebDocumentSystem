...
public class GetImgText : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        var CaptchaText = BusinessObjects.SecurityHelper.DecryptString(
            Convert.FromBase64String(context.Request.QueryString["CaptchaText"]));
        if (CaptchaText != null)
        {
            List<BusinessObjects.Letter> letter = new List<BusinessObjects.Letter>();
            int TotalWidth = 0;
            int MaxHeight = 0;
            foreach (char c in CaptchaText)
            {
                var ltr = new BusinessObjects.Letter(c);
                letter.Add(ltr);
